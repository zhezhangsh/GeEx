...
# datatable options
dt.options1 <- list(
  autoWidth = TRUE, caseInsensitve = TRUE, regex = TRUE, pageLength = 50, sScrollX = TRUE,
  initComplete = DT::JS("function(settings, json) {",
                        "$(this.api().table().header()).css({'background-color': '#666', 'color': '#fff'});", 
                        "}"));
dt.options2 <- list(
  autoWidth = TRUE, caseInsensitve = TRUE, regex = TRUE, pageLength = 5, sScrollX = TRUE, pagingType = 'simple',
  initComplete = DT::JS("function(settings, json) {",
                        "$(this.api().table().header()).css({'background-color': '#666', 'color': '#fff'});", 
                        "}"));
